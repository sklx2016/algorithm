...
def hasPath(matrix, rows, cols, path):
    ...
    for row in range(rows):
        for col in range(cols):
            if hasPath(matrix, rows, cols, row, col, path, pathLength, visited):
                return True
    return False
...
def hasPathCore(matrix, rows, cols, row, col, path, pathLength, visited):
    if len(path) == pathLength:
        return True
    hasPath = False
    if row >= 0 and row < rows and col >= 0 and col <= cols and matrix[row * cols + col] == path[pathLength] \
            and not visited[row * cols + col]:
        pathLength += 1
        visited[row * cols + col] = 1
        hasPath = hasPathCore(matrix, rows, cols, row - 1, col, path, pathLength, visited) or \
